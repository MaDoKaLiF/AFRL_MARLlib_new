...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        other_pos = [np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100])]
        ...
        for i, other in enumerate(world.agents):
            if other is agent:
                ...
                continue
            other_pos[i]=(other.state.p_pos - agent.state.p_pos) #index자리에 추가
            # other_pos.append(world.get_relpos(other, agent))
        
        # relpos of other landmarks
        landmark_rpos = [np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100]),np.array([-100,100])]
        for entity in world.landmarks:  # world.entities:
            landmark_rpos[i]=(entity.state.p_pos - agent.state.p_pos) #index자리에 추가
