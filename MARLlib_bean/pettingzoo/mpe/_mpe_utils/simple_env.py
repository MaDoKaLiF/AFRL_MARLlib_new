...
class SimpleEnv(AECEnv):
    ...
    def _execute_world_step(self):
        ...
        self.world.step()
        self.scenario.update_que(self.world, self, self.np_random)
        ...
        for agent in self.world.agents: 
            if agent.name in self.world.del_agent_que: continue #skip rewarding agents such that will be deleted just right after
    ...
    def update_agents(self): # Update agents according to self.world.del_agent_que, self.world.new_agent_que 
        ...
        for agent in self.world.agents:
            if agent.name in self.world.del_agent_que:
                # remove deleted agents' info
                del self.rewards[agent.name]
                del self._cumulative_rewards[agent.name]
                del self.terminations[agent.name]
                del self.truncations[agent.name]
                del self.infos[agent.name]

                # remove deleted agents' spaces
                del self.observation_spaces[agent.name]
                del self.action_spaces[agent.name]
            else:
        ...
        ## add new agents properties (dict)
        for agent in self.world.new_agent_que:
            self.rewards[agent.name] = 0.0
            self._cumulative_rewards[agent.name] = 0.0
            self.terminations[agent.name] = False
            self.truncations[agent.name] = False
            self.infos[agent.name] = {}

        # self.world.calc_distmat()

        # update new agents spaces
